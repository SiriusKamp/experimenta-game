...
public bool isTouchingWall = false;
 ...
 private Vector2 rayDirection;
    ...
    public Transform arrowSpawnPoint; // Ponto de origem da flecha.
...
{
    // Define o parâmetro "IsAttacking" no Animator de volta para falso.
    animator.SetBool("IsAttacking", false);
    cd = Time.time + 0.5;
    IsAttacking = false;
 // Instancia a flecha no ponto de origem adequado.
            Vector3 arrowPosition = arrowSpawnPoint.position;
            if (spriteRenderer.flipX)
            {
                // Se o jogador estiver virado para a esquerda, ajuste a posição da flecha e inverta sua escala.
                arrowPosition.x -= 1.3f;
                arrowPrefab.transform.localScale = new Vector3(-1, 1, 1);
            }
            else
            {
                // Se o jogador estiver virado para a direita, a flecha permanece na posição padrão.
                arrowPrefab.transform.localScale = Vector3.one;
                arrowPosition.x += 1.3f;
            }
            Instantiate(arrowPrefab, arrowPosition, Quaternion.identity);
...
if (Input.GetKeyDown(KeyCode.Mouse0) && Time.time > cd) // Certifique-se de que não está já atacando.
    {
        // Define o parâmetro "IsAttacking" no Animator como verdadeiro.
                   if(AttackSound != null)
